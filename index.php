...
<header>
	<div class="top">
		<div class="basewidth">
			...
			<nav>
				<ul>
					<li class="sele"><a href="index.php">首页</a></li>
	...
	<div class="main basewidth">
		...
		<div class="leftside">
			<?php
				$num = 10;
				$m = $num * ($cur_page - 1);
				$total = mysql_query("SELECT count(*) FROM artical");
				$artical_total = mysql_fetch_array($total);
			?>
			<?php if($result = mysql_query("SELECT * FROM artical order by articalID desc limit $m,10")){ ?>
				<?php while($row = mysql_fetch_array($result)) { ?>
					<section datao-role="" class="datalist">
						...
						<h4 class="datatitle"><a href="http://happyonly.com.cn/view.php?id=<?php echo $row["articalID"];?>"><?php echo urldecode($row["title"]);?></a></h4>
						...
						<div class="morebtn"><a href="view.php?id=<?php echo $row["articalID"];?>">阅读全文</a></div>
					</section>
				<?php } ?>
			<?php } ?>
			<div class="fen_page"><?php ch_page($artical_total[0], 2, $cur_page, "http://happyonly.com.cn/index.php?p=");?></div>
...
$("#Quiet").click(function(){
	$.get("http://happyonly.com.cn/package/logout.php",function(result){
